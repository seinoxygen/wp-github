...
function ghprofile_shortcode($atts) {
	...
	$html .=  '<a target="_blank" href="'. $profile->html_url . '" title="View ' . $username . '\'s Github"><img src="http://gravatar.com/avatar/' . $profile->gravatar_id . '?s=56" alt="View ' . $username . '\'s Github" /></a>';
	...
	$html .=  '</div>';
	$html .=  '<a target="_blank" class="wpgithub-bblock" href="https://github.com/' . $username . '?tab=repositories"><span class="wpgithub-count">' . $profile->public_repos . '</span><span class="wpgithub-text">Public Repos</span></a>';
	$html .=  '<a target="_blank" class="wpgithub-bblock" href="https://gist.github.com/' . $username . '"><span class="wpgithub-count">' . $profile->public_gists . '</span><span class="wpgithub-text">Public Gists</span></a>';
	$html .=  '<a target="_blank" class="wpgithub-bblock" href="https://github.com/' . $username . '/followers"><span class="wpgithub-count">' . $profile->followers . '</span><span class="wpgithub-text">Followers</span></a>';
	$html .= '</div>';
	return $html;
}
...
function ghcontents_shortcode($atts) {
	...
	$file_name =  str_replace('/','.',$filepath);
	// Init the cache system.
	$cache = new WpGithubCache();
	// Set custom timeout in seconds.
	$cache->timeout = get_option('wpgithub_cache_time', 600);
	...
	if($data_cache == null) {
		$github = new Github($username, $repository, $filepath);
	...
	$html = '<pre class="wp-github line-numbers language-'.$language.'"><code class="language-'.$language.'">';
		if(!isset($contents->message) ):
			...
			$html .= 'User : '.$username. '<br />Repo : '.$repository.'<br />file : '.$file_name;
		endif;
